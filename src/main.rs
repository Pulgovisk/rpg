...
// SFML stuff
use sfml::graphics::{
    Color, IntRect, RectangleShape, RenderTarget, RenderWindow, Shape, Sprite, Texture,
    Transformable,
};
use sfml::system::{SfBox, Vector2};
        ...
        for render_target in &level.render_targets {
            ...
            for point in render_target.1 {
                ...
                if texture_info.rigid.is_some() && texture_info.rigid.unwrap() {
                    let mut shape = RectangleShape::with_size(Vector2::new(
                        (texture_info.w - BOX_FRAME_SIZE * 2) as f32,
                        (texture_info.h - BOX_FRAME_SIZE * 2) as f32,
                    ));
                    shape.set_position(Vector2::new(
                        tx + BOX_FRAME_SIZE as f32,
                        ty + BOX_FRAME_SIZE as f32,
                    ));
